...
beforeEach(()=>{
  ...
  it('Deve fazer login com sucesso', () =>{
    ...
    cy.get('.woocommerce-form > :nth-child(2) > [name="password"]').type('teste@teste.com', {log: false})
  ...
  it('Senha inválida', () => {
    cy.get('[name="username"]').type('teste_aluno11@teste.com')
    cy.get('.woocommerce-form > :nth-child(2) > [name="password"]').type('testeteste.com', {log: false})
    cy.get('[name="login"]').click()
    cy.get('.woocommerce-error > li').should('contain', 'Erro: A senha fornecida para o e-mail teste_aluno11@teste.com está incorreta. Perdeu a senha?')
